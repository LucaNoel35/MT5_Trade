...
class ConTrader:
    ...
    def performTrade(self):
        ...
        self.execute_trades(-1,1,"price_2","price_1","position_2","position_1",self.gain,self.loss,-1,1)      
            ...
            if  (self.spread <= minimal_pip_multiplier*self.pip and self.spread_average<minimal_avg_pip_multiplier*self.pip and self.counter_buy==self.counter_sell) or self.counter_buy!=self.counter_sell : 
                for position in positions:                       
                    ...
                    #originally buy position     
                        ...
                        if   (strat!=strat_close or hold_invertible==False) and (df.at[ls[-1],"config"]==1*strat_close) and position_taken_2==-1  and ((self.close*self.inverse<self.inverse*price and self.objectif_reached_sell(price_2,loss,loss,mode_close)) or self.close*self.inverse>price*self.inverse) and self.objectif_reached_buy(price,gain,gain,mode_close)  :  
                            #price=self.close
                        ...
                        elif   (strat!=strat_close or hold_invertible==False) and (df.at[ls[-1],"config"]==1*strat_close)  and position_taken_2!=-1 and self.objectif_reached_buy(price,gain,gain,mode_close)  :  
                            #price=self.close
                            self.close_buy_position(position)   
                            position_taken=0
                            #setattr(self,price_name,price)
                    ...
                    #originally sell position    
                        ...
                        if  (strat!=strat_close or hold_invertible==False) and (df.at[ls[-1],"config"]==-1*strat_close)  and position_taken_2==1  and ((self.close*self.inverse>price*self.inverse and self.objectif_reached_buy(price_2,loss,loss,mode_close)) or self.close*self.inverse<price*self.inverse) and self.objectif_reached_sell(price,gain,gain,mode_close) :  
                            #price=self.close
                            ...
                            #setattr(self,position_name,position_taken)
                            self.positions=None
                        elif  (strat!=strat_close or hold_invertible==False) and (df.at[ls[-1],"config"]==-1*strat_close)  and position_taken_2!=1  and self.objectif_reached_sell(price,gain,gain,mode_close) :  
                            #price=self.close
                            self.close_sell_position(position)   
                            position_taken=0
                            #setattr(self,price_name,price)
